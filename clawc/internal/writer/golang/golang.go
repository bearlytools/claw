...
func (w *Writer) WriteFiles(ctx context.Context, config imports.ConfigProvider, renders []render.Rendered) error {
	...
	for _, r := range renders {
		...
		go func() {
			defer wg.Done()
			if config.ShouldWriteFile(r.Path) {
				...
				// Write main file
				mainFile := filepath.Join(p, r.Package+".go")
				if err := w.fs.WriteFile(mainFile, r.Native, 0o600); err != nil {
					errs.add(fmt.Errorf("problem writing package(%s) to local file(%s): %w", r.Package, mainFile, err))
					return
				}
				// Write additional files if any
				for filename, content := range r.Files {
					additionalFile := filepath.Join(p, filename)
					if err := w.fs.WriteFile(additionalFile, content, 0o600); err != nil {
						errs.add(fmt.Errorf("problem writing additional file(%s) for package(%s): %w", filename, r.Package, err))
						return
					}
				}
			}
