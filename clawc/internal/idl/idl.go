...
// ExternalEnumImports returns a deduped list of packages that contain external enums
// used by this file's structs. This is used by clawiter.tmpl to import only the
// packages needed for external enum lookups.
func (f *File) ExternalEnumImports() chan string {
	ch := make(chan string, 1)
	...
	go func() {
		...
		for _, s := range f.Structs() {
			for _, field := range s.Fields {
				if field.IsExternal && field.IsEnum && field.FullPath != "" {
