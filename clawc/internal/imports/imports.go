...
// ShouldWriteFile determines if a file should be written for the given package path.
// For Config (non-vendored), this is the same as InRootRepo.
func (c *Config) ShouldWriteFile(pkgPath string) bool {
	return c.InRootRepo(pkgPath)
}

// TransformImportPath transforms a package path to its proper import path.
// For vendored packages, this prepends the vendor repo and directory.
// For local packages (within VendorRepo), returns the path unchanged.
func (c *Config) TransformImportPath(pkgPath string) string {
	// If not using vendoring, return path as-is
	if c.VendorRepo == "" || c.VendorDir == "" {
		return pkgPath
	}
	// If the package is local (starts with VendorRepo), return as-is
	if strings.HasPrefix(pkgPath, c.VendorRepo) {
		return pkgPath
	}
	// External package - prepend vendor repo and directory
	return path.Join(c.VendorRepo, c.VendorDir, pkgPath)
}
...
// vendorDirPath is the full filesystem path to the vendor directory.
// repoPath is the repo path from claw.work (e.g., "github.com/bearlytools/claw").
// vendorDirName is the vendor directory name from claw.work (e.g., "claw_vendor").
func NewVendoredConfig(vendorDirPath, repoPath, vendorDirName string) *VendoredConfig {
	config := NewConfig()
	config.VendorRepo = repoPath
	config.VendorDir = vendorDirName
...
// ShouldWriteFile determines if a file should be written for the given package path.
// For VendoredConfig, always returns true - we want to write .go files for both
// local packages and vendored packages.
